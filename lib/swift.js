...
class Swift {
  constructor(options = {}) {
    if (options.fieldPatterns) {
      this.fieldPatterns = JSON.parse(fs.readFileSync(
        path.resolve(process.cwd(), options.fieldPatterns),
      ));
  ...
  run() {
    ...
    this.watcher
      .on('add', (filePath) => {
        ...
        promise = Promise.resolve(promise).then(() => new Promise((resolve, reject) => {
